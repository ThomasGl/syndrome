...
mod avx2_kernel {
    ...
    // `unsafe fn` that already carries `#[target_feature(enable = "avx2")]`.
    // Rust 1.87 made those calls safe under that condition — no inner
    // `unsafe` block needed, and no `unused_unsafe` allow to reconcile it
    // against an older MSRV, since this crate's MSRV (see `rust-version` in
    // Cargo.toml) is past 1.87.
