...
//! $\alpha^{k}$ for exactly one $k \in \{0, \dots, 254\}$, called
...
//! $i \in \{0, \dots, \text{parity\_shards}-1\}$ and data column $j \in
//! \{0, \dots, \text{data\_shards}-1\}$ — the same shape as a **Vandermonde
...
//! distinct parity-row indices in $\{0, \dots, \text{parity\_shards}-1\}$
